...
const BookingModal: React.FC<BookingModalProps> = ({
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      try {
        ...
        const response = await fetch(`https://book-my-show-webapp-1.onrender.com/seats/${event.venueid}`);
        ...
        if (!venueSeats || venueSeats.length === 0) {
          ...
          const createSeatsResponse = await fetch('https://book-my-show-webapp-1.onrender.com/createseat', {
            method: 'POST',
            headers: {
          ...
          const newSeatsResponse = await fetch(`https://book-my-show-webapp-1.onrender.com/seats/${event.venueid}`);
        ...
        const seatsWithAvailability = await Promise.all(
          venueSeats.map(async (seat: Seat) => {
            const availabilityResponse = await fetch(
              `https://book-my-show-webapp-1.onrender.com/seats/check/${seat.seatnumber}/${event.venueid}`
            );
  ...
  const handleBookingConfirm = async () => {
    ...
    selectedSeats?.map(async(seat) => {
      ...
      const res = await axios.get(`https://book-my-show-webapp-1.onrender.com/bookseat/${seat.seatid}/${seat.seatnumber}/${event.venueid}`);
      console.log(res);
    });
